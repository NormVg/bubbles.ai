...
# 11. Filesystem Map (ASCII Tree Representation)

## Purpose

The **ASCII Filesystem Map** provides a compact visual representation of the entire memory directory.
It allows the agent to understand the structure of stored memories without reading every file.

This map is generated by the **Capture Tool** and acts as a **navigation layer for memory retrieval**.

Benefits:

* quick discovery of memory locations
* token-efficient context
* faster reasoning about available knowledge
...
The ASCII tree mirrors the **memory directory hierarchy**.
...
Each folder corresponds to a **memory type** described earlier in the architecture.

---

## Metadata Representation

The capture tool may optionally include **lightweight metadata** to help the agent prioritize files.
...
│       ├─ created: 2026-02-01
│       ├─ last_used: 2026-04-15
│       └─ relations: [referrals.md, analytics.md]
│
├── tasks/
│   └── todo.md
│       └─ importance: 9
...
Metadata helps the agent determine:

* which files are frequently used
* which files are important
* what relationships exist between memories.

---

## Agent Usage Flow

When the agent needs context, it follows this process:

1. Call the **capture tool**.
2. Receive the ASCII tree.
3. Identify relevant folders and memory files.
4. Use the **read tool** to retrieve detailed memory.

Example reasoning flow:

```
User: "What progress has been made on Openlist?"

Agent steps:
1. capture()
2. finds: projects/openlist.md
3. read(openlist.md)
4. return latest progress
```
...
Agent --> CaptureTool
CaptureTool --> FileSystem
FileSystem --> TreeBuilder
TreeBuilder --> AsciiTree
AsciiTree --> Agent
```

---

## Optimized Capture Output Example

A richer capture output may include counts and relations:
...
This helps the agent quickly estimate:
...
The ASCII tree acts as a **memory navigation layer** between the agent and the filesystem.

It reduces unnecessary file reads and improves retrieval efficiency.
...
Agent --> Capture
Capture --> AsciiTree
AsciiTree --> AgentDecision
AgentDecision --> ReadMemory
ReadMemory --> MemoryFiles
```
